...
*/
func (qc *QRSigningClientCMS) AddDataToSign(names []string, data string, meta []map[string]string, isPDF bool) {
	if len(names) == 0 {
		panic(&QRSigningError{
...
func main() {
	// Usage example:
	qrSigner := NewQRSigningClientCMS("Тестовое подписание", false, baseURL)

	// Add data to sign (encoded in base64)
	// dataToSignBase64 := "MTEK"
	dataToSignBase64, err := helpers.ReadPdf("someFile.pdf")
	if err != nil {
		fmt.Println("Could not read file: ", err)
		return
	}

	qrSigner.AddDataToSign([]string{"Блок данных на подпись", "Блок данных на подпись", "Блок данных на подпись"}, dataToSignBase64, nil, true)
	...
	*/
	...
	if len(signatures) > 0 {
		...
		documentDataResponse, err := docRes.PostDocumentData(documentRegistrationResponse.DocumentID, []byte(documentRegistrationResponse.Data), baseURL)
		if err != nil {
			fmt.Println("documentDataResponse Error:", err)
			return
		}
		fmt.Println("documentDataResponse:", documentDataResponse)
